...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trunct_length = random.randint(4, int(self.block_size * 7/8))
        truncated_document = document[:trunct_length]
        # masked_length = random.randint(int((1/4) * len(truncated_document))-1, int((1/4) * len(truncated_document))+1)
        masked_length = random.randint(1, int(1/4 * trunct_length) * 2)
        ...
        suffix = truncated_document[-(trunct_length - prefix_length - masked_length):]
        # masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.PAD_CHAR * (len(document) - trunct_length)
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.PAD_CHAR * (self.block_size - (
            len(prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content)
        ))
        # print(len(masked_string))
        input_str = masked_string[:-1]
        output_str = masked_string[1:]
        X = torch.tensor([self.stoi[char] for char in input_str])
        Y = torch.tensor([self.stoi[char] for char in output_str])
        return X, Y 
