...
def naiveSoftmaxLossAndGradient(
    ...
    yw_hat = softmax(np.dot(outsideVectors, centerWordVec))
    # print(yw_hat)
    yw = np.array([0]*len(outsideVectors))
    yw[outsideWordIdx] = 1
    loss = -np.log(yw_hat[outsideWordIdx])
    # print(loss)
    gradCenterVec = np.dot(outsideVectors.T,(-yw + yw_hat))
    # print(centerWordVec.shape)
    # print((yw - yw_hat).shape)
    # gradOutsideVecs = -np.outer(centerWordVec, (yw - yw_hat))
    # gradOutsideVecs = np.outer(centerWordVec, -(yw - yw_hat))
    # gradOutsideVecs = np.outer(centerWordVec, (yw_hat - yw))
    # print(gradOutsideVecs.shape)
    gradOutsideVecs = np.outer((yw_hat - yw), centerWordVec)
    # print(gradOutsideVecs.shape)
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    v_c = centerWordVec
    loss = -np.log(sigmoid(np.dot(u_0, v_c))) - np.sum(np.log(sigmoid(-np.dot(u_k, v_c))))
    ...
    unique, unique_idx, unique_counts = np.unique(negSampleWordIndices, return_index=True, return_counts=True)
    ...
    gradOutsideVecs_k = np.outer(
        ...
        v_c
    )[unique_idx]# use unique_idx to access value in negsampleWordIndices
    gradOutsideVecs_k = np.array([vec * count for vec, count in zip(gradOutsideVecs_k, unique_counts)])
    # use unique to access original oursideVec values
    gradOutsideVecs[unique, :] = gradOutsideVecs_k
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    # print(windowSize)
    # print(centerWordVectors.shape)
    # print(outsideVectors.shape)
    # print(word2vecLossAndGradient)
    centerWord_idx = word2Ind[currentCenterWord]
    for outsideword in outsideWords:
        result = word2vecLossAndGradient(
        ...
        loss += result[0]
        gradCenterVecs[centerWord_idx] += result[1]
