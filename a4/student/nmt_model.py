...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # self.h_projection (Linear Layer with no bias), called W_{h} in the PDF.
        # self.h_projection = nn.Linear(hidden_size, hidden_size * 2)
        self.h_projection = nn.Linear(hidden_size * 2, hidden_size, bias=False)
        # self.c_projection (Linear Layer with no bias), called W_{c} in the PDF.
        # self.c_projection = nn.Linear(hidden_size, hidden_size * 2)
        self.c_projection = nn.Linear(hidden_size * 2, hidden_size, bias=False)
        # self.att_projection (Linear Layer with no bias), called W_{attProj} in the PDF.
        # self.att_projection = nn.Linear(hidden_size, hidden_size * 2, bias=False)
        self.att_projection = nn.Linear(hidden_size * 2, hidden_size, bias=False)
        # self.combined_output_projection (Linear Layer with no bias), called W_{u} in the PDF.
        # self.combined_output_projection= nn.Linear(hidden_size, hidden_size * 3, bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        cnnoutput = self.post_embed_cnn(X)
        X = torch.permute(cnnoutput, (2, 0, 1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            Ybar_t = torch.concat((Y_t, o_prev), 1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_state[0], 2))
        ...
        U_t = torch.concat((dec_state[0], a_t), 1)
        # U_t = torch.concat((a_t, dec_state[0]), 1)
