...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        weight = nn.Parameter(torch.empty(self.embed_size * self.n_features, self.hidden_size))
        # bias = nn.Parameter(torch.empty(self.embed_size, self.hidden_size))
        bias = nn.Parameter(torch.empty(self.hidden_size))
        self.embed_to_hidden_weight = weight
        nn.init.xavier_uniform_(weight)
        self.embed_to_hidden_bias = bias
        ...
        ###        Initialize weight with the `nn.init.xavier_uniform_` function and bias with `nn.init.uniform_`
        ###        with default parameters.
        weight = nn.Parameter(torch.empty(self.hidden_size, self.n_classes))
        bias = nn.Parameter(torch.empty(self.n_classes))
        # bias = nn.Parameter(torch.empty(self.hidden_size, self.n_classes))
        self.hidden_to_logits_weight = weight
        nn.init.xavier_uniform_(weight)
        self.hidden_to_logits_bias = bias
        nn.init.uniform_(bias)
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.cat(
                [torch.flatten(torch.index_select(self.embeddings, 0, w[i, :])) for i in np.arange(w.shape[0])]
            ).view(w.shape[0], self.n_features * self.embed_size)
            # ).view(w.shape[0], w.shape[1] * self.embed_size)
    ...
    def forward(self, w):
        ...
        layer1 = torch.matmul(self.embedding_lookup(w), self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        relu = nn.ReLU()
        hidden_output = relu(layer1)
